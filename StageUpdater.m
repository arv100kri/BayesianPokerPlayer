    ...
    for player = 1:N
        ...
        if player ~= info.cur_pos % info.cur_pos is just the agent when the call comes to this function
          for stage = 0:info.stage-1
            %there will be atleast one occurence of history.bet for stages < info.stage
            startRound = info.history.stage_starts(stage+1);
            endRound = size(info.history.bet, 1);
            if stage ~= info.stage - 1
              endRound = info.history.stage_starts(stage+2)-1;
            end
            
            for round = startRound:endRound
            
              if info.history.bet(round, player) == CALL
                featureVector(NUMCALLSTOTAL) = featureVector(NUMCALLSTOTAL) + 1;
              elseif info.history.bet(round, player) == RAISE
                featureVector(NUMRAISESTOTAL) = featureVector(NUMRAISESTOTAL) + 1;
              end
              
              if round > startRound && info.history.bet(round, player) == RAISE && info.history.bet(round -1, player) == CALL
                featureVector(NUMCHECKRAISESTOTAL) = featureVector(NUMCHECKRAISESTOTAL) + 1;
              elseif round > startRound && info.history.bet(round, player) == RAISE && info.history.bet(round -1, player) == RAISE
          ...
          % info.history.bet is updated for all the previous stages.
